...
function FeedbackPage() {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        const apiUrl = "http://localhost:8080/doctor/findAllDoctors";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Example: Submit to backend API
      /*
      const submitResponse = await fetch('http://localhost:8080/feedback/submit', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
